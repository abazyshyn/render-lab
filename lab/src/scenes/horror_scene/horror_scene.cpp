...
namespace Lab
{
    ...
    CHorrorScene::CHorrorScene()
        : m_Window(CWindow::GetInstance()),
          ...
          m_OpaqueSceneEntities({std::make_shared<CGround>(CModel(Utils::LAB_BASE_MODELS_PATH + "sponza_horror_scene/scene.gltf"))}),
    ...
    void CHorrorScene::OnUpdate(float t_DeltaTime)
    {
        ...
        // Process opaque scene entities // TODO: right now it is only "ground"
        m_SceneShader.Bind();
